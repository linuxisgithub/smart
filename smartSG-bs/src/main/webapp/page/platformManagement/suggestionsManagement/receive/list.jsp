...
<form id="queryForm">
	<input name="s_business" value="receive">
</form>
</div>
...
$(function() {
	...
	var titles = [
			...
			{label: "处理结果", width: "10%"},
			{label: "删<span class='wzkg'></span>除", width: "10%",visible:ableDel},
		]
	var columns = [
			...
			{field: "status"},
			{fhtml: "<img src='${ctx}/images/icons/clear_icon.png'/>", func: toDelete,visible:ableDel},
		]
...
function toEdit(page){
	if(page.item.msgType == "reply"){
		...
		parent.$.prompt.layerUrl("${ctx}/suggestions/receive/load.htm?eid=" + page.item.eid);
	}
}
