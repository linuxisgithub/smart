...
if(src.lastIndexOf("?") != -1) {
	...
	var params = _type.split("&");
	$.each(params, function(index, title) {
		var map = title.split("=");
		if(map[1] == "settings") {
			type = "_settings";
		}
		if(map[0] == "search" && map[1] == 1) {
			hasSearch = true;
		}
	});
...
document.write("<link rel='stylesheet' href='"+_path+"/tools/twbsPagination/css_new/pagination"+type+".css' />");
...
if(hasSearch) {
	document.write("<script type='text/javascript' src='"+_path+"/tools/twbsPagination/js/search.js'></script>");
}
	...
	$.ajax({
		...
		'success':function(response,statusText){
			if(response.status == 200) {
				...
				defaultOpts.totalRecords = response.total;
				defaultOpts.items = {};
				$.each(response.data, function(index, item) {
					...
					$.each(columns, function(_index, column) {
						...
						if(typeof(func) != "undefined") {
							...
							if(typeof(column.fcount) != "undefined") {
								$link.text(index + 1 );
								item['fcount'] = index + 1; 
							} else if(typeof(fhtml) != "undefined") {
								$link.append(fhtml);
							} else {
							...
							if(typeof(column.fcount) != "undefined") {
								$td.text(index + 1 ).appendTo(trNode);
								item['fcount'] = index + 1; 
