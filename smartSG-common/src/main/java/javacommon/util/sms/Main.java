...
public class Main {
	...
	public static void main(String[] args) {
		...
		ConfigManager.IS_ENCRYPT_PWD = false;
		boolean isEncryptPwd = ConfigManager.IS_ENCRYPT_PWD;
		// 单条发送
		singleSend(userid, pwd, isEncryptPwd);
		 // 每次请求想要获取的状态报告的最大条数。
		/*int retsizeRpt = 100; // 实例化状态报告线程
		  RecvRptThread recvRptThread = new RecvRptThread(userid, pwd,
		  isEncryptPwd, retsizeRpt); // 线程获取 启动获取状态报告的线程 
		  recvRptThread.start();
		...
		/*
		 * // 相同内容群发 batchSend(userid, pwd, isEncryptPwd);
		 * 
		 * // 个性化群发 multiSend(userid, pwd, isEncryptPwd);
		 * 
		 * // 查询余额 getBalance(userid, pwd, isEncryptPwd);
		 * 
		 * // 查询剩余金额或条数 getRemains(userid, pwd, isEncryptPwd);
		 * 
		 * // 每次请求想要获取的上行的最大条数。 int retsizeMo = 100; // 实例化获取上行线程 RecvMoThread
		 * recvMoThread = new RecvMoThread(userid, pwd, isEncryptPwd,
		 * retsizeMo); // 线程获取 启动获取上行的线程 recvMoThread.start();
		 * System.out.println("获取上行的线程启动成功！");
		 * 
		 * // 每次请求想要获取的状态报告的最大条数。 int retsizeRpt = 100; // 实例化状态报告线程
		 * RecvRptThread recvRptThread = new RecvRptThread(userid, pwd,
		 * isEncryptPwd, retsizeRpt); // 线程获取 启动获取状态报告的线程 recvRptThread.start();
		 * System.out.println("获取状态报告的线程启动成功！");
		 */
	...
	public static void singleSend(String userid, String pwd,
		...
		try {
			...
			// 设置手机号码 此处只能设置一个手机号码
			message.setMobile("13640635965");
			// 设置内容
			message.setContent("同事您好，感谢您对此次测试的配合。123456");
			// 设置扩展号
			message.setExno("");
			// 用户自定义流水编号
			message.setCustid("");
	...
	public static void multiSend(String userid, String pwd, boolean isEncryptPwd) {
		try {
			...
			// 发送短信
			result = cHttpPost.multiSend(userid, pwd, timestamp, multixMts,
					msgId);
			// result为0:成功;非0:失败
			if (result == 0) {
	 ...
	 *            密码是否加密 true：密码加密;false：密码不加密
	 */
	public static void getBalance(String userid, String pwd,
			boolean isEncryptPwd) {
		// 返回值
		int result = -310099;
		try {
