...
public class CHttpPost {
	 ...
	 * @return 0:成功 非0:返回错误代码
	 */
	public int multiSend(String userId, String password, String timestamp,
			List<MultiMt> multiMtList, StringBuffer msgId) {
		// 定义返回值，默认个性化群发失败
		int returnInt = ERROR_310099;
		...
		try {
			...
			// 短连接个性化群发
			returnInt = sendSmsByNotKeepAlive("multi_send", message,
					messageBuffer);
			...
			if (Message != null && !"".equals(Message.trim())) {
				...
				if (returnInt == 0) {
					...
					// 平台返回的custid
					rCustid = (String) parseObject.get("custid");
					msgId.append(multiMtList.get(0).getMobile() + "," + rCustid
							+ "," + rMsgid);
				}
			}
		} catch (Exception e) {
	...
	public int getBalance(String userId, String password, String timestamp) {
		...
		try {
			...
			resultBalance = getMoRptFeeByNotKeepAlive("get_balance", message,
					messageBuffer);
			...
			if (Message != null && !"".equals(Message.trim())) {
				JSONObject parseObject = (JSONObject) JSONValue.parse(Message);
				// 获取是否成功标识
				int result = Integer.parseInt(String.valueOf(parseObject
						.get("result")));
				// result为0，则查询余额成功
				if (result == 0) {
					// 短信余额
					resultBalance = Integer.parseInt(String.valueOf(parseObject
							.get("balance")));
				} else {
					// result不为0，则是错误码
					// 将返回值设置为错误码
	 ...
	 * @return 0:成功;非0:返回错误代码
	 */
	public int getMo(String userId, String password, String timestamp,
			int retsize, List<MO> mos) {
		int returnInt = ERROR_310099;
		try {
			...
			String Message = null;
			StringBuffer messageBuffer = new StringBuffer("");
			returnInt = getMoRptFeeByNotKeepAlive("get_mo", message,
					messageBuffer);
			...
			if (nearMO.getResult() == 0) {
				...
				for (int i = 0; i < mos.size(); i++) {
					mo = mos.get(i);
					// 解码
					mos.get(i).setContent(
							URLDecoder.decode(mo.getContent(), "UTF-8"));
				}
	 ...
	 * @return 0:成功;非0:返回错误代码
	 */
	public int getRpt(String userId, String password, String timestamp,
			int retsize, List<RPT> rpts) {
		int returnInt = ERROR_310099;
		try {
			Message message = new Message();
			...
			returnInt = getMoRptFeeByNotKeepAlive("get_rpt", message,
					messageBuffer);
	...
	public Remains getRemains(String userId, String password, String timestamp) {
		...
		try {
			...
			int resultInt = getMoRptFeeByNotKeepAlive("get_balance", message,
					messageBuffer);
			...
			if (Message != null && !"".equals(Message.trim())) {
				...
				if (result == 0) {
					// 获取计费类型
					int chargetype = Integer.parseInt(String
							.valueOf(parseObject.get("chargetype")));
					// 设置计费类型
					remains.setChargetype(chargetype);

					// 剩余条数
					int balance = Integer.parseInt(String.valueOf(parseObject
							.get("balance")));
					// 设置剩余条数
